...
export const fetchTeams = async () => {
    ...
    try {
        const response = await fetch('https://api.pandascore.co/players?sort=birthday&page=1&per_page=50', options); // Se modifica la url si se quiere un filtro distinto
        if (!response.ok) {
                ...
                <div className="teams-container">
                    ...
                    <ul>
                        {Array.isArray(teams) && teams.length > 0 ? (
                            ...
                            <p>Cargando todos los jugadores...</p> // Mensaje de carga, si no funciona la carga de equipo se quedará ese mensaje en la pantalla
                        )}
                    </ul>
                </div>
                <Footer />
